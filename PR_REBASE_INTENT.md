...
Kyverno v1.19 is the last release that supports ClusterPolicy and CleanupPolicy types.
...
Once we remove the legacy engine and code, all open PRs (<https://github.com/kyverno/kyverno/pulls>) that reference the old code wil show a conflict.
...
1. For each open PR, check if it makes changes to legacy policy type code, new CEL policy type code, or both:
   ...
   3. If its for both, either split to two PRs or keep to main
