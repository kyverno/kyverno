...
| resources | object | `{"limits":{"memory":"384Mi"},"requests":{"cpu":"100m","memory":"128Mi"}}` | Existing Image pull secrets for image verify and imageData policies. This will define the `--imagePullSecrets` Kyverno argument. existingImagePullSecrets: [] Define an existing image pull secret existingImagePullSecrets: - private-registry - test-registry |
