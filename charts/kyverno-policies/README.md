...
| podSecurityUserNamespaces | bool | `false` | Enable support for Kubernetes User Namespaces (hostUsers: false). When enabled, the disallow-proc-mount policy will allow pods with spec.hostUsers set to false, as user namespaces map container UID 0 to a non-root host UID, relaxing the Baseline procMount requirement. Requires Kubernetes v1.33+ with the UserNamespacesPodSecurityStandards feature gate. For more info: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/ |
