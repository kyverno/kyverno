...
| podSecurityRelaxForUserNamespacePods | bool | `true` | When `true`, the `require-run-as-nonroot`, `require-run-as-non-root-user`, and `disallow-proc-mount` Pod Security Standards policies automatically skip pods that opt into Kubernetes user namespaces (`spec.hostUsers: false`), matching the relaxation defined for Pod Security Admission integration. Applies to both the `ClusterPolicy` and `ValidatingPolicy` variants of these policies. See: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/#integration-with-pod-security-admission-checks |
