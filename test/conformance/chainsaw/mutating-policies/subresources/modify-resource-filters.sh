...
get_resource_filters() {
    kubectl get ConfigMap kyverno -n kyverno -o json | \
    python3 -c 'import sys, json; print(json.load(sys.stdin)["data"]["resourceFilters"])'
}
...
if [ "$1" = "removeNode" ]; then
    ...
    kubectl patch ConfigMap kyverno -n kyverno \
        --type='json' \
        -p="[{\"op\": \"replace\", \"path\": \"/data/resourceFilters\", \"value\": \"$resource_filters\"}]"
fi
