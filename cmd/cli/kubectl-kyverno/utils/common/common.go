...
func ApplyPolicyOnResource(c ApplyPolicyConfig) ([]*api.EngineResponse, Info, error) {
...
func ProcessValidateEngineResponse(policy kyvernov1.PolicyInterface, validateResponse *api.EngineResponse, resPath string, rc *ResultCounts, policyReport bool, auditWarn bool) Info {
...
func buildPVInfo(er *api.EngineResponse, violatedRules []kyvernov1.ViolatedRule) Info {
	info := Info{
...
func updateResultCounts(policy kyvernov1.PolicyInterface, engineResponse *api.EngineResponse, resPath string, rc *ResultCounts, auditWarn bool) {
	printCount := 0
	for _, policyRule := range autogen.ComputeRules(policy) {
		ruleFoundInEngineResponse := false
...
func processMutateEngineResponse(c ApplyPolicyConfig, mutateResponse *api.EngineResponse, resPath string) error {
	...
	for _, policyRule := range autogen.ComputeRules(c.Policy) {
		...
		for i, mutateResponseRule := range mutateResponse.PolicyResponse.Rules {
			if policyRule.Name == mutateResponseRule.Name {
				ruleFoundInEngineResponse = true
				if mutateResponseRule.Status == api.RuleStatusPass {
					c.Rc.Pass++
					printMutatedRes = true
				} else if mutateResponseRule.Status == api.RuleStatusSkip {
					fmt.Printf("\nskipped mutate policy %s -> resource %s", c.Policy.GetName(), resPath)
					c.Rc.Skip++
				} else if mutateResponseRule.Status == api.RuleStatusError {
...
func handleGeneratePolicy(generateResponse *api.EngineResponse, policyContext api.PolicyContext, ruleToCloneSourceResource map[string]string) ([]api.RuleResponse, error) {
