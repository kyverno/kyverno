...
func printReports(out io.Writer, engineResponses []engineapi.EngineResponse, auditWarn, outputJson bool) {
...
func printExceptions(out io.Writer, engineResponses []engineapi.EngineResponse, auditWarn, outputJson bool, ttl time.Duration) {
	clustered, _ := report.ComputePolicyReports(auditWarn, engineResponses...)
	for _, report := range clustered {
		for _, result := range report.Results {
			if result.Result == "fail" {
				if err := printException(out, result, ttl, outputJson); err != nil {
					log.Error(err)
				}
			}
		}
...
func printException(out io.Writer, result v1alpha2.PolicyReportResult, ttl time.Duration, outputJson bool) error {
