...
// formatResource converts a comma-separated resource key into a display-friendly
// slash-separated path. Standard keys have exactly 4 comma-separated parts:
// apiVersion, kind, namespace, and name. For cluster-scoped resources where namespace
// is empty, the empty component is omitted to avoid producing double slashes.
// Non-standard keys (fewer or more than 4 parts) fall back to simple formatting.
func formatResource(resourceKey string) string {
	parts := strings.Split(resourceKey, ",")
	if len(parts) != 4 {
		// Fallback if not a standard 4-part GVK/namespace/name key
