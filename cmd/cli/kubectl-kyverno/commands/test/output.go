...
func printFailedTestResult(out io.Writer, resultsTable table.Table, detailedResults bool) error {
	// Create a copy of RawRows to avoid mutating the caller's slice
	copiedRows := make([]table.Row, len(resultsTable.RawRows))
	copy(copiedRows, resultsTable.RawRows)

	for i := range copiedRows {
		copiedRows[i].ID = i + 1
	}

	// Create a new table with the copied rows
	localTable := table.Table{
		RawRows: copiedRows,
	}

	if _, err := fmt.Fprintln(out, "Aggregated Failed Test Cases:"); err != nil {
		return err
	}
	return table.Print(out, localTable, detailedResults)
}
