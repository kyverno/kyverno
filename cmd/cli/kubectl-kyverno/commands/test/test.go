	...
	for _, policy := range results.Policies {
		for _, rule := range autogen.Default.ComputeRules(policy, "") {
			for _, res := range testCase.Test.Results {
				if res.IsValidatingAdmissionPolicy || res.IsValidatingPolicy || res.IsImageValidatingPolicy || res.IsMutatingAdmissionPolicy || res.IsDeletingPolicy || res.IsGeneratingPolicy || res.IsMutatingPolicy {
					continue
				}
