...
func runTest(out io.Writer, testCase test.TestCase, registryAccess bool) (*TestResponse, error) {
	...
	for _, resource := range uniques {
		...
		resourceKey := resource.GetAPIVersion() + "/" + resource.GetName()
		engineResponses = append(engineResponses, ers...)
		testResponse.Trigger[resourceKey] = engineResponses
	}
	for _, targetResource := range targetResources {
		for _, engineResponse := range engineResponses {
			if r, _ := extractPatchedTargetFromEngineResponse(targetResource.GetName(), engineResponse); r != nil {
				// todo: check gvk and name properly
				resourceKey := targetResource.GetAPIVersion() + "/" + targetResource.GetName()
