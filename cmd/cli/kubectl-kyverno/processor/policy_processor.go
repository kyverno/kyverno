...
func (p *PolicyProcessor) ApplyPoliciesOnResource() ([]engineapi.EngineResponse, error) {
	...
	if len(p.ValidatingPolicies) != 0 || len(p.NamespacedValidatingPolicies) != 0 {
		...
		for i := range p.ValidatingPolicies {
			policy := &p.ValidatingPolicies[i]
			if policy.GetValidatingPolicySpec().EvaluationMode() == policieskyvernoio.EvaluationModeJSON {
				jsonPolicies = append(jsonPolicies, policy)
			} else {
				k8sPolicies = append(k8sPolicies, policy)
			}
		}
		for i := range p.NamespacedValidatingPolicies {
			policy := &p.NamespacedValidatingPolicies[i]
			if policy.GetValidatingPolicySpec().EvaluationMode() == policieskyvernoio.EvaluationModeJSON {
				jsonPolicies = append(jsonPolicies, policy)
			} else {
				k8sPolicies = append(k8sPolicies, policy)
		...
		if resource.Object != nil {
			...
			if len(jsonPolicies) > 0 {
				...
				// JSON mode evaluates directly against raw JSON and must not require GVK/GVR resolution.
				// Previously, CLI routed all ValidatingPolicies through admission-style requests, which
				// forced RESTMapper lookups and left JSON-mode policies with a nil object (silent skips).
		...
		if p.JsonPayload.Object != nil && len(jsonPolicies) > 0 {
			provider, err := vpolengine.NewProvider(compiler, jsonPolicies, p.CELExceptions)
			if err != nil {
				return nil, err
			}
