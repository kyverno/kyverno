...
func (p *PolicyProcessor) ApplyPoliciesOnResource() ([]engineapi.EngineResponse, error) {
	...
	// authz policies (Envoy)
	if len(p.EnvoyPolicies) > 0 {
		for _, req := range p.EnvoyRequests {
			for _, vpol := range p.EnvoyPolicies {
				resp, err := processEnvoyPolicy(vpol, req, p.Client)
				if err != nil {
					return nil, err
				}
	...
	// authz policies (HTTP)
	if len(p.HTTPPolicies) > 0 {
		for _, req := range p.HTTPRequests {
			for _, vpol := range p.HTTPPolicies {
				resp, err := processHTTPPolicy(vpol, req, p.Client)
				if err != nil {
					return nil, err
				}
				p.Rc.AddValidatingPolicyResponse(resp)
...
func LoadEnvoyRequests(content string) ([]*authv3.CheckRequest, error) {
...
func LoadHTTPRequests(content string) ([]*authzhttp.CheckRequest, error) {
	trimmed := strings.TrimSpace(content)
	if strings.HasPrefix(trimmed, "[") {
		var raw []json.RawMessage
		if err := json.Unmarshal([]byte(content), &raw); err != nil {
		...
		payload := make([]*authzhttp.CheckRequest, len(raw))
		for i, r := range raw {
	...
	return []*authzhttp.CheckRequest{&p}, nil
}
