				...
				if test.Policy == policyName {
					...
					for _, resource := range test.Resources {
						...
						if name[len(name)-1] == resourceName {
							...
							resultsKey = GetResultKeyAccordingToTestResults(userDefinedPolicyNamespace, test.Policy, test.Rule, test.Namespace, name[2], name[len(name)-1])
	...
	for i, v := range testResults {
		...
		if v.Resources != nil {
			for _, resource := range v.Resources {
				name := strings.FieldsFunc(resource.Object, Split)
				res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(name[2]) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
				...
				resultKey := fmt.Sprintf("%s-%s-%s-%s", v.Policy, ruleNameInResultKey, name[2], name[len(name)-1])
				found, _ := isNamespacedPolicy(v.Policy)
				var ns string
				ns, v.Policy = getUserDefinedPolicyNameAndNamespace(v.Policy)
				if found && v.Namespace != "" {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, v.Namespace, name[2], name[len(name)-1])
				} else if found {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, name[2], name[len(name)-1])
					res.Policy = boldFgCyan.Sprintf(ns) + "/" + boldFgCyan.Sprintf(v.Policy)
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(name[2]) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
				} else if v.Namespace != "" {
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(name[2]) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", v.Policy, ruleNameInResultKey, v.Namespace, name[2], name[len(name)-1])
				}
