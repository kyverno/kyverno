...
func buildPolicyResults(engineResponses []*response.EngineResponse, testResults []kyvernov1.Results, infos []policyreport.Info, policyResourcePath string, fs billy.Filesystem, isGit bool) (map[string]policyreportv1alpha2.PolicyReportResult, []kyvernov1.Results) {
			...
			if test.Resources != nil {
				if test.Policy == policyName {
					...
					for _, resource := range test.Resources {
						fmt.Printf("working\n")
						if resource.Old == resourceName {
							fmt.Printf("working\n")
							var resultsKey string
							resultsKey = GetResultKeyAccordingToTestResults(userDefinedPolicyNamespace, test.Policy, test.Rule, test.Namespace, test.Kind, resource.Old)
							fmt.Printf("resultKey : %v", resultsKey)
				...
				for _, key := range resultsKey {
					for _, resource := range test.Resources {
						...
						if rule.Status == response.RuleStatusSkip {
							...
							x = getAndCompareResource(resource.Generated, rule.GeneratedResource, isGit, policyResourcePath, fs, true)
							if x == "pass" {
...
func applyPoliciesFromPath(fs billy.Filesystem, policyBytes []byte, isGit bool, policyResourcePath string, rc *resultCounts, openAPIController *openapi.Controller, tf *testFilter) (err error) {
	...
	if tf.enabled {
		...
		for _, res := range values.Spec.Results {
			for _, resources := range res.Resources {
				if (len(tf.policy) == 0 || tf.policy == res.Policy) && (len(tf.resource) == 0 || tf.resource == resources.Old) && (len(tf.rule) == 0 || tf.rule == res.Rule) {
	...
	// var userInfoMap map[string]kyvernov1beta1.RequestInfo
	// var subjectInfoMap map[string]store.Subject
	// userInfo := kyvernov1beta1.RequestInfo{}
	...
	resourceFullPath := getFullPath(values.Spec.Resources.My_resource_pool, policyResourcePath, isGit)

	for i, result := range values.Spec.Results {
		for j, resources := range result.Resources {
			arrPatchedResource := []string{resources.Patched}
			arrGeneratedResource := []string{resources.Generated}
			arrCloneSourceResource := []string{resources.CloneSource}
			...
			values.Spec.Results[i].Resources[j].Patched = patchedResourceFullPath[0]
			values.Spec.Results[i].Resources[j].Generated = generatedResourceFullPath[0]
			values.Spec.Results[i].Resources[j].CloneSource = CloneSourceResourceFullPath[0]
		}
	}
	...
	resultsMap, testResults := buildPolicyResults(engineResponses, values.Spec.Results, pvInfos, policyResourcePath, fs, isGit)
...
func CheckVariableForPolicyn(valuesMap map[string]map[string]kyvernov1.Resourcev, globalValMap map[string]string, policyName string, resourceName string, resourceKind string, kindOnwhichPolicyIsApplied map[string]struct{}, variable string) (map[string]interface{}, error) {
	...
	if _, ok := kindOnwhichPolicyIsApplied[resourceKind]; ok {
		if len(variable) > 0 && len(thisPolicyResourceValues) == 0 && len(GetContext().Policies) == 0 {
	...
	for i, v := range testResults {
		...
		if v.Resources != nil {
			for _, resource := range v.Resources {
				res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(resource.Old)
				...
				resultKey := fmt.Sprintf("%s-%s-%s-%s", v.Policy, ruleNameInResultKey, v.Kind, resource.Old)
				found, _ := isNamespacedPolicy(v.Policy)
				var ns string
				ns, v.Policy = getUserDefinedPolicyNameAndNamespace(v.Policy)
				if found && v.Namespace != "" {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, v.Namespace, v.Kind, resource.Old)
				} else if found {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, v.Kind, resource.Old)
					res.Policy = boldFgCyan.Sprintf(ns) + "/" + boldFgCyan.Sprintf(v.Policy)
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(resource.Old)
				} else if v.Namespace != "" {
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(resource.Old)
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", v.Policy, ruleNameInResultKey, v.Namespace, v.Kind, resource.Old)
				}
