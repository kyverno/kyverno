			...
			if test.Resources != nil {
				if test.Policy == policyName {
					...
					for _, resource := range test.Resources {
						name := strings.FieldsFunc(resource.Old, Split)
						if name[len(name)-1] == resourceName {
							var resultsKey string
							resultsKey = GetResultKeyAccordingToTestResults(userDefinedPolicyNamespace, test.Policy, test.Rule, test.Namespace, test.Kind, name[len(name)-1])
								...
								if name[1] == r.GroupVersionKind().Version && name[len(name)-2] == r.GetNamespace() && name[len(name)-1] == r.GetName() {
									if len(name) == 5 {
										if r.GroupVersionKind().Group == name[2] {
											...
											x = getAndCompareResource(r, rule.GeneratedResource, isGit, policyResourcePath, fs, false, resourcesMap)
											if x == "pass" {
										...
										if r.GroupVersionKind().Version == name[1] && r.GetNamespace() == name[2] && r.GetName() == name[3] {
											result.Result = policyreportv1alpha2.StatusFail
											x = getAndCompareResource(r, rule.GeneratedResource, isGit, policyResourcePath, fs, false, resourcesMap)
											if x == "pass" {
												result.Result = policyreportv1alpha2.StatusPass
	...
	for i, v := range testResults {
		...
		if v.Resources != nil {
			for _, resource := range v.Resources {
				name := strings.FieldsFunc(resource.Old, Split)
				res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
				...
				resultKey := fmt.Sprintf("%s-%s-%s-%s", v.Policy, ruleNameInResultKey, v.Kind, name[len(name)-1])
				found, _ := isNamespacedPolicy(v.Policy)
				var ns string
				ns, v.Policy = getUserDefinedPolicyNameAndNamespace(v.Policy)
				if found && v.Namespace != "" {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, v.Namespace, v.Kind, name[len(name)-1])
				} else if found {
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", ns, v.Policy, ruleNameInResultKey, v.Kind, name[len(name)-1])
					res.Policy = boldFgCyan.Sprintf(ns) + "/" + boldFgCyan.Sprintf(v.Policy)
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
				} else if v.Namespace != "" {
					res.Resource = boldFgCyan.Sprintf(v.Namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(name[len(name)-1])
					resultKey = fmt.Sprintf("%s-%s-%s-%s-%s", v.Policy, ruleNameInResultKey, v.Namespace, v.Kind, name[len(name)-1])
				}
