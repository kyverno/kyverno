...
func applyPoliciesFromPath(
	...
	var filteredValidatingAdmissionPolicies []v1alpha1.ValidatingAdmissionPolicy
	for _, p := range validatingAdmissionPolicies {
		for _, res := range values.Results {
			if p.GetName() == res.Policy {
				filteredValidatingAdmissionPolicies = append(filteredValidatingAdmissionPolicies, p)
				break
			}
		}
	}
