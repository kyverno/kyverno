...
func testCommandExecute(
	...
	for _, p := range policies {
		...
		} else if rows, err := printTestResult(reports, tests, rc, failOnly, removeColor); err != nil {
	...
	if rc.Fail > 0 && !failOnly {
		printFailedTestResult(removeColor, table)
		os.Exit(1)
	}
	os.Exit(0)
	return rc, nil
...
func printTestResult(resps map[string]policyreportv1alpha2.PolicyReportResult, testResults []api.TestResults, rc *resultCounts, failOnly, removeColor bool) ([]Row, error) {
	...
	for _, v := range testResults {
		...
		row.Policy = colorize(removeColor, boldFgCyan, v.Policy)
		row.Rule = colorize(removeColor, boldFgCyan, v.Rule)

		if v.Resources != nil {
			for _, resource := range v.Resources {
				row.ID = testCount
				testCount++
				row.Resource = colorize(removeColor, boldFgCyan, v.Namespace) + "/" + colorize(removeColor, boldFgCyan, v.Kind) + "/" + colorize(removeColor, boldFgCyan, resource)
				...
				if found && v.Namespace != "" {
					...
					row.Policy = colorize(removeColor, boldFgCyan, ns) + "/" + colorize(removeColor, boldFgCyan, v.Policy)
					row.Resource = colorize(removeColor, boldFgCyan, v.Namespace) + "/" + colorize(removeColor, boldFgCyan, v.Kind) + "/" + colorize(removeColor, boldFgCyan, resource)
				} else if v.Namespace != "" {
					row.Resource = colorize(removeColor, boldFgCyan, v.Namespace) + "/" + colorize(removeColor, boldFgCyan, v.Kind) + "/" + colorize(removeColor, boldFgCyan, resource)
				...
				if val, ok := resps[resultKey]; ok {
					...
					row.Result = colorize(removeColor, boldYellow, "Not found")
					rc.Fail++
					row.isFailure = true
				...
				if testRes.Result == v.Result {
					row.Result = colorize(removeColor, boldGreen, "Pass")
					...
					row.Result = colorize(removeColor, boldRed, "Fail")
					rc.Fail++
					row.isFailure = true
				}
			...
			row.Resource = colorize(removeColor, boldFgCyan, v.Namespace) + "/" + colorize(removeColor, boldFgCyan, v.Kind) + "/" + colorize(removeColor, boldFgCyan, v.Resource)
			var ruleNameInResultKey string
			if v.AutoGeneratedRule != "" {
				ruleNameInResultKey = fmt.Sprintf("%s-%s", v.AutoGeneratedRule, v.Rule)
			} else {
