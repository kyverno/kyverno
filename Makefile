...
codegen-helm-crds: $(KUSTOMIZE) codegen-crds-all ## Generate helm CRDs
	...
	@$(KUSTOMIZE) build ./config/.helm | $(KUSTOMIZE) cfg grep kind=CustomResourceDefinition |  $(SED) -e "1i{{- if .Values.installCRDs }}" -e '$$a{{- end }}' -e '/^  creationTimestamp: null/i \ \ \ \ {{- toYaml .Values.extraCRDAnnotations | nindent 4 }}' > ./charts/kyverno/templates/crds.yaml
