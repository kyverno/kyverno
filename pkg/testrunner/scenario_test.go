...
func Test_parse_yaml(t *testing.T) {
	...
	assert.Equal(t, api.RuleStatusFail, s.Expected.Validation.PolicyResponse.Rules[0].Status, "invalid status")
}
...
func Test_parse_file(t *testing.T) {
	...
	assert.Equal(t, api.RuleStatusFail, s.TestCases[0].Expected.Validation.PolicyResponse.Rules[0].Status, "invalid status")
}
