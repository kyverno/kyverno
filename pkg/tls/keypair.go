...
func ParseKeyAlgorithm(s string) (KeyAlgorithm, error) {
	...
	case "ED25519":
		return Ed25519, nil
	default:
		return "", fmt.Errorf("unsupported key algorithm: %s (supported: RSA, ECDSA, Ed25519)", s)
	}
}
...
func generateCA(key crypto.PrivateKey, certValidityDuration time.Duration, algorithm KeyAlgorithm) (crypto.PrivateKey, *x509.Certificate, error) {
	...
	der, err := x509.CreateCertificate(rand.Reader, templ, templ, publicKey, privateKey)
	if err != nil {
		return nil, nil, err
	}
