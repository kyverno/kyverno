...
// This file contains the legacy cosign verifier implementation preserved for backward compatibility.
// It is used when the UnifiedImageVerifiers feature flag is disabled.
...
// Legacy signature algorithm map
var legacySignatureAlgorithmMap = map[string]crypto.Hash{
	"":       crypto.SHA256,
...
// legacyCosignVerifier is the original cosign verifier implementation
type legacyCosignVerifier struct{}

// cosignVerifier is a type alias for backward compatibility with tests
type cosignVerifier = legacyCosignVerifier

func newLegacyVerifier() images.ImageVerifier {
	return &legacyCosignVerifier{}
}

func (v *legacyCosignVerifier) VerifySignature(ctx context.Context, opts images.Options) (*images.Response, error) {
...
func buildCosignOptions(ctx context.Context, opts images.Options) (*cosign.CheckOpts, error) {
	...
	signatureAlgorithm, ok := legacySignatureAlgorithmMap[opts.SignatureAlgorithm]
	if !ok {
...
func (v *legacyCosignVerifier) FetchAttestations(ctx context.Context, opts images.Options) (*images.Response, error) {
	if opts.SigstoreBundle {
		results, err := verifyBundleAndFetchAttestations(ctx, opts)
		if err != nil {
			return nil, err
