...
func Test_CheckKind(t *testing.T) {
	...
	// Though both 'pods', 'pods/status' have same kind i.e. 'Pod', when no subresource is specified in the kind selector,
	// it should match the resource regardless of subresources. This allows exceptions and exclusions for "Pod" to apply to all Pod operations.
	...
	// Though both 'pods', 'pods/ephemeralcontainers' have same kind i.e. 'Pod', when no subresource is specified,
	// it should match regardless. The allowEphemeralContainers flag is now redundant but kept for backward compatibility.
	...
	match = CheckKind([]string{"Pod/exec"}, schema.GroupVersionKind{Kind: "Pod", Group: "", Version: "v1"}, "exec", false)
	assert.Equal(t, match, true)

	match = CheckKind([]string{"Pod/exec"}, schema.GroupVersionKind{Kind: "Pod", Group: "", Version: "v1"}, "log", false)
	assert.Equal(t, match, false)

	match = CheckKind([]string{"Pod/*"}, schema.GroupVersionKind{Kind: "Pod", Group: "", Version: "v1"}, "exec", false)
	assert.Equal(t, match, true)

	match = CheckKind([]string{"Pod/*"}, schema.GroupVersionKind{Kind: "Pod", Group: "", Version: "v1"}, "log", false)
	assert.Equal(t, match, true)
