...
func newPolicyResponse(policy, rule string, patchesStr []string, status api.RuleStatus) api.PolicyResponse {
	var patches [][]byte
...
func newEngineResponse(policy, rule string, patchesStr []string, status api.RuleStatus, annotation map[string]interface{}) *api.EngineResponse {
...
func Test_empty_annotation(t *testing.T) {
	...
	engineResponse := newEngineResponse("mutate-container", "default-imagepullpolicy", []string{patchStr}, api.RuleStatusPass, nil)
	annPatches := GenerateAnnotationPatches([]*api.EngineResponse{engineResponse}, logging.GlobalLogger())
...
func Test_exist_annotation(t *testing.T) {
	...
	patchStr := `{ "op": "replace", "path": "/spec/containers/0/imagePullPolicy", "value": "IfNotPresent" }`
	engineResponse := newEngineResponse("mutate-container", "default-imagepullpolicy", []string{patchStr}, api.RuleStatusPass, annotation)
	annPatches := GenerateAnnotationPatches([]*api.EngineResponse{engineResponse}, logging.GlobalLogger())
...
func Test_annotation_nil_patch(t *testing.T) {
	...
	engineResponse := newEngineResponse("mutate-container", "default-imagepullpolicy", nil, api.RuleStatusPass, annotation)
	annPatches := GenerateAnnotationPatches([]*api.EngineResponse{engineResponse}, logging.GlobalLogger())
	assert.Assert(t, annPatches == nil)
	engineResponseNew := newEngineResponse("mutate-container", "default-imagepullpolicy", []string{""}, api.RuleStatusPass, annotation)
	annPatchesNew := GenerateAnnotationPatches([]*api.EngineResponse{engineResponseNew}, logging.GlobalLogger())
...
func Test_annotation_failed_Patch(t *testing.T) {
	...
	engineResponse := newEngineResponse("mutate-container", "default-imagepullpolicy", nil, api.RuleStatusFail, annotation)
	annPatches := GenerateAnnotationPatches([]*api.EngineResponse{engineResponse}, logging.GlobalLogger())
	assert.Assert(t, annPatches == nil)
}
