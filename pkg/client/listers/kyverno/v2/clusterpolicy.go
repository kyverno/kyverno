...
type ClusterPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v2.ClusterPolicy, err error)
	// Get retrieves the ClusterPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.ClusterPolicy, error)
...
// List lists all ClusterPolicies in the indexer.
func (s *clusterPolicyLister) List(selector labels.Selector) (ret []*v2.ClusterPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.ClusterPolicy))
	})
	return ret, err
}

// Get retrieves the ClusterPolicy from the index for a given name.
func (s *clusterPolicyLister) Get(name string) (*v2.ClusterPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("clusterpolicy"), name)
	}
	return obj.(*v2.ClusterPolicy), nil
}
