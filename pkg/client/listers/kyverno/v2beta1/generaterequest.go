...
type GenerateRequestLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta1.GenerateRequest, err error)
...
// List lists all GenerateRequests in the indexer.
func (s *generateRequestLister) List(selector labels.Selector) (ret []*v2beta1.GenerateRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta1.GenerateRequest))
	})
	return ret, err
}
...
type GenerateRequestNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta1.GenerateRequest, error)
...
// List lists all GenerateRequests in the indexer for a given namespace.
func (s generateRequestNamespaceLister) List(selector labels.Selector) (ret []*v2beta1.GenerateRequest, err error) {
...
// Get retrieves the GenerateRequest from the indexer for a given namespace and name.
func (s generateRequestNamespaceLister) Get(name string) (*v2beta1.GenerateRequest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta1.Resource("generaterequest"), name)
	}
	return obj.(*v2beta1.GenerateRequest), nil
}
