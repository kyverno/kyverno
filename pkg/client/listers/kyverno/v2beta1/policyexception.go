...
type PolicyExceptionLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta1.PolicyException, err error)
...
// List lists all PolicyExceptions in the indexer.
func (s *policyExceptionLister) List(selector labels.Selector) (ret []*v2beta1.PolicyException, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta1.PolicyException))
	})
	return ret, err
}
...
type PolicyExceptionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta1.PolicyException, error)
...
// List lists all PolicyExceptions in the indexer for a given namespace.
func (s policyExceptionNamespaceLister) List(selector labels.Selector) (ret []*v2beta1.PolicyException, err error) {
...
// Get retrieves the PolicyException from the indexer for a given namespace and name.
func (s policyExceptionNamespaceLister) Get(name string) (*v2beta1.PolicyException, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta1.Resource("policyexception"), name)
	}
	return obj.(*v2beta1.PolicyException), nil
}
