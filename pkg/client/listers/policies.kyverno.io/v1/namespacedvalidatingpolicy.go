...
type NamespacedValidatingPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*policieskyvernoiov1.NamespacedValidatingPolicy, err error)
...
type namespacedValidatingPolicyLister struct {
	listers.ResourceIndexer[*policieskyvernoiov1.NamespacedValidatingPolicy]
}
...
func NewNamespacedValidatingPolicyLister(indexer cache.Indexer) NamespacedValidatingPolicyLister {
	return &namespacedValidatingPolicyLister{listers.New[*policieskyvernoiov1.NamespacedValidatingPolicy](indexer, policieskyvernoiov1.Resource("namespacedvalidatingpolicy"))}
}
...
func (s *namespacedValidatingPolicyLister) NamespacedValidatingPolicies(namespace string) NamespacedValidatingPolicyNamespaceLister {
	return namespacedValidatingPolicyNamespaceLister{listers.NewNamespaced[*policieskyvernoiov1.NamespacedValidatingPolicy](s.ResourceIndexer, namespace)}
}
...
type NamespacedValidatingPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*policieskyvernoiov1.NamespacedValidatingPolicy, error)
