...
type ValidatingPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*policieskyvernoiov1.ValidatingPolicy, err error)
	// Get retrieves the ValidatingPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*policieskyvernoiov1.ValidatingPolicy, error)
...
type validatingPolicyLister struct {
	listers.ResourceIndexer[*policieskyvernoiov1.ValidatingPolicy]
}
...
func NewValidatingPolicyLister(indexer cache.Indexer) ValidatingPolicyLister {
	return &validatingPolicyLister{listers.New[*policieskyvernoiov1.ValidatingPolicy](indexer, policieskyvernoiov1.Resource("validatingpolicy"))}
}
