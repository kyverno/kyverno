...
type ImageValidatingPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*policieskyvernoiov1.ImageValidatingPolicy, err error)
	// Get retrieves the ImageValidatingPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*policieskyvernoiov1.ImageValidatingPolicy, error)
...
type imageValidatingPolicyLister struct {
	listers.ResourceIndexer[*policieskyvernoiov1.ImageValidatingPolicy]
}
...
func NewImageValidatingPolicyLister(indexer cache.Indexer) ImageValidatingPolicyLister {
	return &imageValidatingPolicyLister{listers.New[*policieskyvernoiov1.ImageValidatingPolicy](indexer, policieskyvernoiov1.Resource("imagevalidatingpolicy"))}
}
