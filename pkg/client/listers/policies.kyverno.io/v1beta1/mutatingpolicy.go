...
type MutatingPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*policieskyvernoiov1beta1.MutatingPolicy, err error)
	// Get retrieves the MutatingPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*policieskyvernoiov1beta1.MutatingPolicy, error)
...
type mutatingPolicyLister struct {
	listers.ResourceIndexer[*policieskyvernoiov1beta1.MutatingPolicy]
}
...
func NewMutatingPolicyLister(indexer cache.Indexer) MutatingPolicyLister {
	return &mutatingPolicyLister{listers.New[*policieskyvernoiov1beta1.MutatingPolicy](indexer, policieskyvernoiov1beta1.Resource("mutatingpolicy"))}
}
