	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.KyvernoV2().Policies(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.KyvernoV2().Policies(namespace).Watch(context.TODO(), options)
			},
		},
		&kyvernov2.Policy{},
		resyncPeriod,
...
func (f *policyInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&kyvernov2.Policy{}, f.defaultInformer)
}

func (f *policyInformer) Lister() v2.PolicyLister {
	return v2.NewPolicyLister(f.Informer().GetIndexer())
}
