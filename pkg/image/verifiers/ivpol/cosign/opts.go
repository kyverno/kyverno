...
func checkOptions(ctx context.Context, att *v1beta1.Cosign, baseROpts []remote.Option, baseNOpts []name.Option, secretLister k8scorev1.SecretInterface) (*cosign.CheckOpts, error) {
	tufMu.Lock()
	defer tufMu.Unlock()
...
func sourceRemoteOpts(ctx context.Context, secretLister k8scorev1.SecretInterface, src *v1beta1.Source) ([]remote.Option, error) {
	...
	if len(src.SignaturePullSecrets) > 0 {
		...
		lister := registryclient.SecretListerFromInterface(secretLister, config.KyvernoNamespace())
		kc := regcreds.NewSecretsKeychain(lister, config.KyvernoNamespace(), signaturePullSecrets...)
