...
// DeleteDownstreams deletes all downstream resources generated by a policy in case the trgger resource is deleted.
// It will not stop the watchers, but will delete the generated resources and remove them from the metadata cache.
func (wm *WatchManager) DeleteDownstreams(policyName string) {
	wm.lock.Lock()
	defer wm.lock.Unlock()
...
func (wm *WatchManager) RemoveWatchersForPolicy(policyName string) {
	wm.lock.Lock()
	defer wm.lock.Unlock()
