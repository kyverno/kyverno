...
func (wm *WatchManager) SyncWatchers(policyName string, triggerUID string, generatedResources []*unstructured.Unstructured) error {
	wm.lock.Lock()
	defer wm.lock.Unlock()

	logger := wm.log
	validUIDs := make(map[types.UID]bool)

	// start a new watcher for each generated resource
	for _, resource := range generatedResources {
		validUIDs[resource.GetUID()] = true
	...
	// Clean up old resources for THIS trigger that are no longer generated
	for _, dw := range wm.dynamicWatchers {
		var uidsToDelete []types.UID
		for uid, resource := range dw.metadataCache {
			if resource.Labels[common.GeneratePolicyLabel] == policyName && resource.Labels[common.GenerateTriggerUIDLabel] == triggerUID {
				if !validUIDs[uid] {
		...
		for _, uid := range uidsToDelete {
			...
			if err != nil {
				...
				delete(dw.metadataCache, uid)
			}
		}
	}

	// Recompute GVRs used by this policy across all triggers
	newGVRsForPolicy := make(map[schema.GroupVersionResource]bool)
	for gvr, dw := range wm.dynamicWatchers {
		for _, resource := range dw.metadataCache {
			if resource.Labels[common.GeneratePolicyLabel] == policyName {
				newGVRsForPolicy[gvr] = true
				break
			}
		}
	}
	...
	for gvr := range newGVRsForPolicy {
		found := false
	...
	// stop old watchers if the policy no longer generates any resources for that GVR
	for _, oldGVR := range oldGVRs {
		if !newGVRsForPolicy[oldGVR] {
			if _, counted := wm.refCount[oldGVR]; counted {
				wm.refCount[oldGVR]--
			}
			if wm.refCount[oldGVR] <= 0 {
				if dw, exists := wm.dynamicWatchers[oldGVR]; exists {
					if dw.watcher != nil {
						dw.watcher.Stop()
					}
	...
	wm.policyRefs[policyName] = make([]schema.GroupVersionResource, 0, len(newGVRsForPolicy))
	for gvr := range newGVRsForPolicy {
