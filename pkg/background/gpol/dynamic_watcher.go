...
func (wm *WatchManager) SyncWatchers(policyName string, generatedResources []*unstructured.Unstructured) error {
	...
	// Build newGVRs first (deduplicating GVRs when a policy generates multiple resources
	// of the same kind) then derive toStart from unique keys, so startWatcher is called
	// at most once per GVR even if generatedResources contains duplicates.
	wm.lock.Lock()
	...
	for _, res := range generatedResources {
		...
		newGVRs[mapping.Resource] = struct{}{}
	}
...
func (wm *WatchManager) handleAdd(obj *unstructured.Unstructured, gvr schema.GroupVersionResource) {
	...
	policyName := obj.GetLabels()[common.GeneratePolicyLabel]
	if policyName == "" {
		return
	}
	wm.lock.Lock()
	defer wm.lock.Unlock()
	// Only track resources for policies that have an active watcher registration.
	// This prevents a shared GVR watcher (kept alive by policy B) from tracking
	// resources generated by policy A when A has synchronization disabled — which
	// would cause handleDelete to recreate those resources despite sync being off.
	if !slices.Contains(wm.policyRefs[policyName], gvr) {
		wm.log.V(4).Info("ignoring add for policy with no active watcher", "name", obj.GetName(), "policy", policyName)
		return
	}
...
// isSyncEnabled, if non-nil, is called for each policy name found in the cluster; resources
// owned by a policy for which it returns false are excluded so that Bootstrap does not restore
// watchers for sync-disabled policies (which would protect their downstreams from deletion).
func (wm *WatchManager) Bootstrap(ctx context.Context, isSyncEnabled func(policyName string) bool) {
	...
	for _, rl := range resourceLists {
		...
		for _, apiRes := range rl.APIResources {
			...
			// Filter to sync-enabled policies so Bootstrap does not restore watchers
			// that would protect downstreams owned by sync-disabled policies.
			var syncedItems []unstructured.Unstructured
			for i := range list.Items {
				policyName := list.Items[i].GetLabels()[common.GeneratePolicyLabel]
				if policyName == "" {
					continue
				}
				if isSyncEnabled != nil && !isSyncEnabled(policyName) {
					continue
				}
				syncedItems = append(syncedItems, list.Items[i])
			}
			if len(syncedItems) == 0 {
				continue
			}
			...
			// startWatcher seeds initialCache from ALL labeled resources for this GVR.
			// Prune entries for sync-disabled policies so handleDelete does not recreate
			// resources that should not be protected.
			if isSyncEnabled != nil {
				for uid, res := range w.metadataCache {
					pName := res.Labels[common.GeneratePolicyLabel]
					if pName != "" && !isSyncEnabled(pName) {
						delete(w.metadataCache, uid)
					}
				}
			}

			// Rebuild policyRefs and refCount from sync-enabled resource labels so that
			...
			wm.log.V(2).Info("bootstrap: watcher restored", "gvr", gvr, "resources", len(syncedItems))
		}
	}
