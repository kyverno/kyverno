...
func NewProcessor(client dclient.Interface, kyvernoClient versioned.Interface, mpolEngine mpolengine.Engine, mapper meta.RESTMapper, context libs.Context, reportsConfig reportutils.ReportingConfiguration, reportsBreaker breaker.Breaker, statusControl common.StatusControlInterface) *processor {
	return &processor{
		client:         client,
...
func (p *processor) Process(ur *kyvernov2.UpdateRequest) error {
	...
	for _, target := range targets.Items {
		...
		if er := p.createBgReports(object, &response); er != nil {
			logger.Error(er, "failed to create background reports for mpol", "mpol", ur.Spec.GetPolicyKey())
		}
...
func (p *processor) createBgReports(object *unstructured.Unstructured, response *mpolengine.EngineResponse) error {
	...
	report := reportutils.BuildBackgroundScanReport(object.GetNamespace(), object.GroupVersionKind(), object.GetName(), object.GetUID(), engineResponses...)
