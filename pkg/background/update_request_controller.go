...
func (c *controller) updatePolicy(_, obj interface{}) {
	key, err := cache.MetaNamespaceKeyFunc(obj)
	if err != nil {
		logger.V(4).Info("updating update requests for policy", "key", key)
		urs, err := c.urLister.GetUpdateRequestsForClusterPolicy(key)
		if err != nil {
			logger.Error(err, "failed to fetch update request for policy", "key", key)
			return
		}
		...
		logger.Error(err, "failed to compute policy key", "obj", obj)
	}
}
