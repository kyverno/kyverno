...
func (c *MutateExistingController) ProcessUR(ur *kyvernov1beta1.UpdateRequest) error {
	...
	for _, rule := range policy.GetSpec().Rules {
		...
		for _, r := range er.PolicyResponse.Rules {
			...
			switch r.Status {
			case api.RuleStatusFail, api.RuleStatusError, api.RuleStatusWarn:
...
func addAnnotation(policy kyvernov1.PolicyInterface, patched *unstructured.Unstructured, r api.RuleResponse) (patchedNew *unstructured.Unstructured, err error) {
	if patched == nil {
		return
	}
