...
func applyCommandHelper(resourcePaths []string, cluster bool, policyReport bool, mutateLogPath string,
	...
	for _, policy := range mutatedPolicies {
		...
		for _, resource := range resources {
			...
			_, _, info, err := common.ApplyPolicyOnResource(policy, resource, mutateLogPath, mutateLogPathIsDir, thisPolicyResourceValues, policyReport, namespaceSelectorMap, stdin, rc, true)
			if err != nil {
