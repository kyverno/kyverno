...
func applyPoliciesFromPath(fs billy.Filesystem, policyBytes []byte, isGit bool, policyResourcePath string, rc *resultCounts, openAPIController *openapi.Controller, tf *testFilter) (err error) {
	...
	for i, result := range values.Results {
		...
		arrCloneResource := []string{result.CloneResource}
		...
		cloneResourceFullPath := getFullPath(arrCloneResource, policyResourcePath, isGit)

		values.Results[i].PatchedResource = patchedResourceFullPath[0]
		values.Results[i].GeneratedResource = generatedResourceFullPath[0]
		values.Results[i].CloneResource = cloneResourceFullPath[0]
	}
	...
	var ruleToCloneResource = map[string]string{}
				...
				if rule.Name == res.Rule {
					...
					if rule.HasGenerate() {
						...
						if genClone != nil && len(genClone) != 0 {
							ruleToCloneResource[rule.Name] = res.CloneResource
						}
					}
					break
				}
	...
	for _, policy := range mutatedPolicies {
		...
		for _, resource := range resources {
			...
			ers, info, err := common.ApplyPolicyOnResource(policy, resource, "", false, thisPolicyResourceValues, true, namespaceSelectorMap, false, &resultCounts, false, ruleToCloneResource)
			if err != nil {
