...
func printTestResult(resps map[string]report.PolicyReportResult, testResults []TestResults, rc *resultCounts) error {
	...
	for i, v := range testResults {
		...
		if v.Resources != nil {
			for _, resource := range v.Resources {
				res.Resource = boldFgCyan.Sprintf(namespace) + "/" + boldFgCyan.Sprintf(v.Kind) + "/" + boldFgCyan.Sprintf(resource)
	...
	if countDeprecatedResource > 0 {
		fmt.Printf("\n Note : The resource field is being deprecated in 1.7.0 release. Please provide the resources under the resources parameter as an array in the results field \n")
	}
