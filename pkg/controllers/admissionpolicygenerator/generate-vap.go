...
func (c *controller) handleVAPGeneration(ctx context.Context, polType string, policy engineapi.GenericPolicy) error {
	...
	if polType == "ClusterPolicy" {
		...
		if !wantVap {
			...
			// delete any previously generated autogen VAPs
	...
	// For ValidatingPolicy with autogen VAP generation enabled, also generate VAPs for
	// each autogenerated pod-controller variant so that webhook registration for those
	// kinds can be skipped (they are governed by native VAPs instead).
	if vpol := policy.AsValidatingPolicy(); vpol != nil && vpol.GetSpec().GenerateValidatingAdmissionPolicyEnabled() {
		return c.handleAutogenVAPs(ctx, vpol, genericExceptions)
	}
