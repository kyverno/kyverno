...
func NewController(
	...
	if mapAlphaInformer != nil {
		...
		if _, err := controllerutils.AddEventHandlersT(mapAlphaInformer.Informer(), c.addMAP, c.updateMAP, c.deleteMAP); err != nil {
			logger.Error(err, "failed to register event handlers")
		}
	}
	...
	if mapbindingAlphaInformer != nil {
		...
		if _, err := controllerutils.AddEventHandlersT(mapbindingAlphaInformer.Informer(), c.addMAPbinding, c.updateMAPbinding, c.deleteMAPbinding); err != nil {
			logger.Error(err, "failed to register event handlers")
		}
	}
