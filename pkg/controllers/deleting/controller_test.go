...
func (m *mockDiscovery) RESTMapper() meta.RESTMapper {
	return m.restMapper
}
...
func (m *mockDClient) Discovery() dclient.IDiscovery {
	apiGroupResources, err := restmapper.GetAPIGroupResources(m.kube.Discovery())
	if err != nil {
		return &mockDiscovery{restMapper: meta.NewDefaultRESTMapper(nil)}
	}
	return &mockDiscovery{restMapper: restmapper.NewDiscoveryRESTMapper(apiGroupResources)}
}
...
func TestDeleting_ClusterWidePaginationWithoutNamespaceSelector(t *testing.T) {
	...
	// A nil NamespaceSelector means all namespaces. Since there is no
	// namespace restriction, the hybrid implementation uses a cluster-wide
	// resource client and relies on API pagination rather than iterating
	// through namespaces individually.
	assert.NoError(t, err)
