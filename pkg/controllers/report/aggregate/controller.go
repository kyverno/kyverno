...
func NewController(
	...
	polrInformer := metadataFactory.ForResource(reportv1alpha1.SchemeGroupVersion.WithResource("policyreports"))
	cpolrInformer := metadataFactory.ForResource(reportv1alpha1.SchemeGroupVersion.WithResource("clusterpolicyreports"))
	c := controller{
		client:      client,
		dclient:     dclient,
...
func (c *controller) getReport(ctx context.Context, namespace, name string) (reportsv1.ReportInterface, error) {
	if namespace == "" {
		report, err := c.client.OpenreportsV1alpha1().ClusterReports().Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			if apierrors.IsNotFound(err) {
				return nil, nil
			}
		...
		report, err := c.client.OpenreportsV1alpha1().Reports(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			if apierrors.IsNotFound(err) {
				return nil, nil
			}
...
func (c *controller) backReconcile(ctx context.Context, logger logr.Logger, _, namespace, name string) (err error) {
	...
	merged := map[string]reportv1alpha1.ReportResult{}
	mergeReports(maps, merged, types.UID(name), reports...)
	results := make([]reportv1alpha1.ReportResult, 0, len(merged))
