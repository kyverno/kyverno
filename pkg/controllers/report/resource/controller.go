...
type MetadataCache interface {
	GetResourceHash(types.UID, ...schema.GroupVersionKind) (Resource, schema.GroupVersionKind, bool)
...
func (c *controller) GetResourceHash(uid types.UID, gvks ...schema.GroupVersionKind) (Resource, schema.GroupVersionKind, bool) {
	c.lock.RLock()
		...
		if err != nil {
			...
			go func(gvr schema.GroupVersionResource) {
				...
				for event := range watchInterface.ResultChan() {
					logger.Info("event", "type", event.Type, "gvr", gvr)
					switch event.Type {
					case watch.Added:
...
func (c *controller) updateHash(eventType EventType, obj *unstructured.Unstructured, gvr schema.GroupVersionResource) {
	...
	if exists {
		...
		if exists && hash != watcher.hashes[uid].Hash {
			...
			logger.Info("hash", "hash", hash, "gvk", watcher.gvk)
