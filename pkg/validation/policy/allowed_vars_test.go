...
func TestNotAllowedVars_MatchSection(t *testing.T) {
  ...
  `)
	...
	policy, _, _, _, _, _, err := yamlutils.GetPolicy(policyWithVarInMatch)
...
func TestNotAllowedVars_ExcludeSection(t *testing.T) {
  ...
  `)
	...
	policy, _, _, _, _, _, err := yamlutils.GetPolicy(policyWithVarInExclude)
	assert.NilError(t, err)
...
func TestNotAllowedVars_JSONPatchPath_ContextRootPositive(t *testing.T) {
	...
	policy, _, _, _, _, _, err := yamlutils.GetPolicy(policyManifest)
	assert.NilError(t, err)
...
`)
	...
	policy, _, _, _, _, _, err := yamlutils.GetPolicy(policyJSON)
	assert.NilError(t, err)
...
func TestNotAllowedVars_VariableFormats(t *testing.T) {
    ...
    `, tc.input))
		...
		policy, _, _, _, _, _, err := yamlutils.GetPolicy(policyYAML)
		assert.NilError(t, err)
