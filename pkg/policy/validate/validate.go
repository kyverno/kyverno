...
type Validate struct {
	...
	rule *kyvernov2beta1.Validation
}

// NewValidateFactory returns a new instance of Mutate validation checker
func NewValidateFactory(rule *kyvernov2beta1.Validation) *Validate {
	m := Validate{
		rule: rule,
	}
...
func validationElemCount(v *kyvernov2beta1.Validation) int {
	if v == nil {
		return 0
	}
...
func (v *Validate) validateForEach(foreach kyvernov2beta1.ForEachValidation) error {
	if foreach.List == "" {
...
func foreachElemCount(foreach kyvernov2beta1.ForEachValidation) int {
	count := 0
