...
func (pc *policyController) createURForNamespacedGeneratingPolicy(ngpol *policiesv1beta1.NamespacedGeneratingPolicy) error {
	...
	if len(downstreams) == 0 {
		...
		triggers := filterTriggersByNamespace(pc.getGpolTriggers(ngpol), ngpol.GetNamespace())
		for _, trigger := range triggers {
...
func (pc *policyController) getGpolTriggers(gpol policiesv1beta1.GeneratingPolicyLike) []*unstructured.Unstructured {
	...
	if pc.gpolEngine != nil && pc.gpolProvider != nil {
		policyKey := gpol.GetName()
		if gpol.GetNamespace() != "" {
			policyKey = gpol.GetNamespace() + "/" + gpol.GetName()
		}
		celPolicy, celFetchErr = pc.gpolProvider.Get(context.TODO(), policyKey)
	}
