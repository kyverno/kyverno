...
func (pc *policyController) createURForGeneratingPolicy(gpol *policiesv1beta1.GeneratingPolicy) error {
	...
	if len(downstreams) == 0 {
		...
		for _, downstream := range downstreams {
			...
			addRuleContext(ur, gpol.GetName(), trigger, false, synchronize, false)
		}
		// invalidate the downstream resources that are generated by the gpol from the cache
		pc.watchManager.InvalidateDownstreams(gpol.GetName(), nil)
	}
...
func (pc *policyController) createURForNamespacedGeneratingPolicy(ngpol *policiesv1beta1.NamespacedGeneratingPolicy) error {
	...
	if len(downstreams) == 0 {
		...
		for _, downstream := range downstreams {
			...
			// fetch the trigger resource labels from the downstream resource to build the UR
			trigger := generateutils.TriggerFromLabels(labels)
			addRuleContext(ur, policyKey, trigger, false, synchronize, false)
		}
		// invalidate the downstream resources that are generated by the ngpol from the cache
		pc.watchManager.InvalidateDownstreams(policyKey, nil)
	}
	if len(ur.Spec.RuleContext) == 0 {
		return nil
	}
