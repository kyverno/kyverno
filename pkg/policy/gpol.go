...
func (pc *policyController) createURForGeneratingPolicy(gpol *policiesv1beta1.GeneratingPolicy) error {
	...
	if len(downstreams) == 0 {
		// No cached downstreams — either genuinely none exist yet, or RemoveWatchersForPolicy
		// cleared the cache after a spec change.  Use match constraints to find all current
		// trigger resources and create URs with cacheRestore=false so the engine actually
		// calls UpdateResource when the template changed, instead of silently skipping the
		// write and leaving resources with the old spec.
		pc.log.V(4).Info("no downstream resources found in cache for generating policy, syncing from triggers")
