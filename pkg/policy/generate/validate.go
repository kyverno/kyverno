...
func (g *Generate) Validate() (string, error) {
	...
	if rule.ForEachGeneration != nil && (rule.Kind != "" || rule.Name != "" || rule.Namespace != "") {
		err := fmt.Errorf("Generate rule cannot be written individually when foreach specified in Generate block, All the Generate rules must be specified inside the foreach block")
		log.Error(err)
	}
...
func (g *Generate) validateForeach() (string, error) {
	...
	for _, fe := range rule.ForEachGeneration {
		for _, sr := range fe.GenerateSubResources {
			if sr.GetData() != nil && sr.Clone != (kyvernov1.CloneFrom{}) {
				return "", fmt.Errorf("only one of data or clone can be specified")
			}

			if sr.CloneList.Namespace != "" || len(sr.CloneList.Kinds) > 0 {
				return "", fmt.Errorf("CloneList is not supported inside foreach generate")
			}

			kind, namespace := sr.Kind, sr.Namespace

			if !reflect.DeepEqual(sr.Clone, kyvernov1.CloneFrom{}) {
				if path, err := g.validateClone(sr.Clone, sr.CloneList, kind); err != nil {
					return fmt.Sprintf("clone.%s", path), err
				}
			}
			if target := rule.GetData(); target != nil {
