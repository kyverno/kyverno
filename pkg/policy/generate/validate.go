...
type Generate struct {
	...
	rule kyvernov2beta1.Generation
...
func NewGenerateFactory(client dclient.Interface, rule kyvernov2beta1.Generation, log logr.Logger) *Generate {
	g := Generate{
		rule:      rule,
...
func (g *Generate) Validate() (string, error) {
	rule := g.rule
	if rule.GetData() != nil && rule.Clone != (kyvernov2beta1.CloneFrom{}) {
	...
	if !reflect.DeepEqual(rule.Clone, kyvernov2beta1.CloneFrom{}) {
...
func (g *Generate) validateClone(c kyvernov2beta1.CloneFrom, kind string) (string, error) {
	if c.Name == "" {
		return "name", fmt.Errorf("name cannot be empty")
	}
