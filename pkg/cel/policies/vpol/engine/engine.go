...
func (e *engineImpl) Handle(ctx context.Context, request EngineRequest, predicate Predicate) (EngineResponse, error) {
	...
	// build the `request` CEL activation value once for the whole loop below,
	// instead of once per policy. BuildRawRequestMap reproduces vpol's base
	// semantics (an un-normalized unmarshal of request.Object.Raw /
	// request.OldObject.Raw) - it does not take object/oldObject, since
	// those are ExtractResources-normalized and must not leak into
	// request.object/request.oldObject here.
	requestMap, err := celcompiler.BuildRawRequestMap(&request.Request)
	if err != nil {
		return response, err
	}
	// evaluate policies
	for _, policy := range policies {
		...
		startTime := time.Now()
		pol := e.handlePolicy(ctx, policy, nil, attr, &request.Request, namespace, requestMap, request.Context)
...
func (e *engineImpl) handlePolicy(ctx context.Context, policy Policy, jsonPayload any, attr admission.Attributes, request *admissionv1.AdmissionRequest, namespace runtime.Object, requestMap map[string]any, context libs.Context) engine.ValidatingPolicyResponse {
	...
	case policy.ExtractionMode:
		...
		// requestMap above - it must not be forwarded here.
		result, err = e.evaluateExtracted(ctx, policy, attr, request, namespace, context)
	default:
		result, err = policy.CompiledPolicy.Evaluate(ctx, nil, attr, request, namespace, requestMap, context)
	}
