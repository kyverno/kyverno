...
func (p *Policy) evaluateWithData(
	...
	allowedValues := make(map[string][]string, 0)
	...
	if len(p.exceptions) > 0 {
		...
		for _, polex := range p.exceptions {
			...
			if match {
				...
				for key, value := range polex.Exception.Spec.AllowedValues {
					if existing, ok := allowedValues[key]; ok {
						allowedValues[key] = append(existing, value...)
					} else {
