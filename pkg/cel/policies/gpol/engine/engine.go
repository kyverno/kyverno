...
func (e *engineImpl) generate(
	...
	context.SetGenerateContext(policy.Policy.GetName(), policy.Policy.GetNamespace(), request.Name, attr.GetNamespace(), request.Kind.Version, request.Kind.Group, request.Kind.Kind, triggerUID, cacheRestore, policy.Policy.GetSpec().UseServerSideApply, policy.LegacyOwnerConflicts)
