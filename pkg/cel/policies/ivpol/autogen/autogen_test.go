...
func TestAutogenMetadataPaths(t *testing.T) {
	policy := ivpol.DeepCopy()
	policy.Spec.AutogenConfiguration.PodControllers.Controllers = []string{"deployments", "cronjobs"}
	...
	policy.Spec.MatchConditions = []admissionregistrationv1.MatchCondition{{
		...
		Expression: `oldObject.metadata.uid != '' && object.metadata.?labels[?'app'].orValue('') == 'demo'`,
	}}
	policy.Spec.Validations = []admissionregistrationv1.Validation{{
		Expression:        `object.metadata.?annotations[?'example.com/key'].orValue('') == 'enabled'`,
		MessageExpression: `object.metadata.name + ':' + oldObject.metadata.resourceVersion`,
	}}
	...
	assert.Equal(t, `oldObject.metadata.uid != '' && object.spec.template.metadata.?labels[?'app'].orValue('') == 'demo'`, deployment.MatchConditions[0].Expression)
	assert.Equal(t, `object.spec.template.metadata.?annotations[?'example.com/key'].orValue('') == 'enabled'`, deployment.Validations[0].Expression)
	assert.Equal(t, `object.metadata.name + ':' + oldObject.metadata.resourceVersion`, deployment.Validations[0].MessageExpression)
	assert.Equal(t, `object.spec.template.spec.containers.map(e, e.image)`, deployment.ImageExtractors[0].Expression)

	cronjob := generated[autogen.AutogenCronjobs].Spec
	assert.Equal(t, `object.spec.jobTemplate.spec.template.metadata.?annotations[?'example.com/key'].orValue('') == 'enabled'`, cronjob.Validations[0].Expression)
	assert.Equal(t, `object.spec.jobTemplate.spec.template.spec.containers.map(e, e.image)`, cronjob.ImageExtractors[0].Expression)
}
