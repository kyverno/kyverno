	...
	for _, data := range dataList {
		...
		for _, item := range items {
			...
			if err != nil && apierrors.IsNotFound(err) {
				...
				// the resource already exists (e.g. a resync, a retry, or a
				// cacheRestore pass over a resource generated by an earlier UR).
				// Report it as generated so callers relying on the generated
				// resources list (e.g. WatchManager.SyncWatchers) see the
				// resource is still managed by this policy, instead of treating
				// it as no longer generated and tearing down its watcher/cache.
