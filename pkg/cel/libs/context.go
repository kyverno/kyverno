	...
	for _, data := range dataList {
		...
		for _, item := range items {
			...
			if err != nil && apierrors.IsNotFound(err) {
				...
				return err
			} else if cp.isManagedByPolicy(existing) {
...
// resource generated by this provider's policy. It is used to avoid
// reporting a pre-existing, unrelated resource (one that merely shares the
// same GVK/namespace/name) as if it were generated by us.
