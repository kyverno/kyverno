		...
		t.Run(tt.name, func(t *testing.T) {
			...
			data := map[string]any{
				"globalContext": Context{&ContextMock{
					GetGlobalReferenceFunc: func(name, projection, jmesPath string) (any, error) {
						ent, ok := mockStore.Get(name)
						if !ok {
							return nil, errors.New("global context entry not found")
						}
						return ent.Get(projection, jmesPath)
					},
				}},
			}
...
func Test_impl_get_string_string_string(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	options := []cel.EnvOption{
	...
	assert.NoError(t, err)
	assert.NotNil(t, env)
	ast, issues := env.Compile(`globalContext.Get("foo", "bar", "baz")`)
	assert.Nil(t, issues)
	assert.NotNil(t, ast)
	prog, err := env.Program(ast)
	assert.NoError(t, err)
...
func Test_impl_get_string_string_string_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	options := []cel.EnvOption{
	...
	tests := []struct {
		...
		{
			name: "bad arg 1",
			args: []ref.Val{types.String("foo"), types.String("foo"), types.String("foo"), types.String("foo")},
			want: types.NewErr("unsupported native conversion from string to 'globalcontext.Context'"),
		},
		{
			name: "bad arg 2",
			args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.Bool(false), types.String("foo"), types.String("foo")},
			want: types.NewErr("type conversion error from bool to 'string'"),
		},
		{
			name: "bad arg 3",
			args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.String("foo"), types.Bool(false), types.String("foo")},
			want: types.NewErr("type conversion error from bool to 'string'"),
		},
		{
			name: "bad arg 4",
			args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.String("foo"), types.String("foo"), types.Bool(false)},
			want: types.NewErr("type conversion error from bool to 'string'"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &impl{}
			got := c.get_string_string_string(tt.args...)
			assert.Equal(t, tt.want, got)
		})
	}
}
