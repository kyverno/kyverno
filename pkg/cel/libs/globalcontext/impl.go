...
func (c *impl) get_string_string_string(args ...ref.Val) ref.Val {
	if len(args) != 4 {
	...
	if self, err := utils.ConvertToNative[Context](args[0]); err != nil {
		...
		return types.WrapErr(err)
	} else if jmesPath, err := utils.ConvertToNative[string](args[3]); err != nil {
		return types.WrapErr(err)
	} else {
		globalRef, err := self.GetGlobalReference(name, projection, jmesPath)
		if err != nil {
			// Errors are not expected here since Parse is a more lenient parser than ParseRequestURI.
			return types.NewErr("failed to get global reference: %v", err)
		}
