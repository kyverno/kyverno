...
func Test_namespaced_impl_get_resource_string_string_string(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotNil(t, env)
	ast, issues := env.Compile(`resource.Get("apps/v1", "deployments", "nginx")`)
	assert.Nil(t, issues)
	assert.NotNil(t, ast)
...
func Test_namespaced_impl_get_resource_string_string_string_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
...
func Test_namespaced_impl_list_resources_string_string_string(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
	...
	assert.NoError(t, err)
	assert.NotNil(t, env)
	ast, issues := env.Compile(`resource.List("apps/v1", "deployments")`)
	assert.Nil(t, issues)
	assert.NotNil(t, ast)
	prog, err := env.Program(ast)
	assert.NoError(t, err)
...
func Test_namespaced_impl_list_resources_string_string_map(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotNil(t, env)
	ast, issues := env.Compile(`resource.List("apps/v1", "deployments", {"app": "nginx"})`)
	assert.Nil(t, issues)
	assert.NotNil(t, ast)
	prog, err := env.Program(ast)
	assert.NoError(t, err)
...
func Test_namespaced_post_resource_string_string_map(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
...
resource.Post(
	...
	data := map[string]any{
		"resource": Context{&ContextMock{
			PostResourceFunc: func(apiVersion, resource, namespace string, payload map[string]any) (*unstructured.Unstructured, error) {
				...
				assert.Equal(t, payload["metadata"].(map[string]any)["namespace"], "default")
	...
	assert.Equal(t, object["metadata"].(map[string]any)["namespace"], "default")
}

func Test_namespaced_impl_post_resource_string_string_map_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
	...
	tests := []struct {
		...
		name: "bad arg 1",
		args: []ref.Val{types.String("foo"), types.String("v1"), types.String("pods"), types.NewMapType(types.StringType, types.AnyType)},
		want: types.NewErr("invalid arg 0: unsupported native conversion from string to 'resource.Context'"),
	}, {
		name: "bad arg 2",
		args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.Bool(false), types.String("pods"), types.NewMapType(types.StringType, types.AnyType)},
		want: types.NewErr("invalid arg 1: type conversion error from bool to 'string'"),
	}, {
		name: "bad arg 3",
		args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.String("v1"), types.Bool(false), types.NewMapType(types.StringType, types.AnyType)},
		want: types.NewErr("invalid arg 2: type conversion error from bool to 'string'"),
	}, {
		name: "bad arg 4",
		args: []ref.Val{env.CELTypeAdapter().NativeToValue(Context{}), types.String("v1"), types.String("pods"), types.Bool(false)},
...
func Test_namespaced_impl_list_resources_forbidden_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
...
func Test_namespaced_impl_get_resource_forbidden_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
...
func Test_namespaced_impl_post_resource_forbidden_error(t *testing.T) {
	base, err := compiler.NewBaseEnv()
	assert.NoError(t, err)
	assert.NotNil(t, base)
	env, err := base.Extend(
