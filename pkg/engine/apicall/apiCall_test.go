...
func Test_APICallConfiguration_GetTimeout(t *testing.T) {
	// Default configuration should have default timeout
	config := NewAPICallConfiguration(1000)
	assert.Equal(t, DefaultAPICallTimeout, config.GetTimeout())

	// Custom timeout configuration
	customTimeout := 10 * time.Second
	config = NewAPICallConfigurationWithTimeout(1000, customTimeout)
