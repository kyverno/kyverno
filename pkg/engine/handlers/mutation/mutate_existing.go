...
func (h mutateExistingHandler) Process(
	...
	if err != nil {
		rr := engineapi.RuleError(rule, engineapi.Mutation, "", err)
	...
	for _, target := range targets {
		...
		if err := contextLoader(ctx, target.context, policyContext.JSONContext()); err != nil {
			rr := engineapi.RuleError(rule, engineapi.Mutation, "failed to load context", err)
			responses = append(responses, *rr)
			continue
		}
		...
		if err != nil {
			rr := engineapi.RuleError(rule, engineapi.Mutation, "failed to evaluate preconditions", err)
			responses = append(responses, *rr)
			continue
		}
		if !preconditionsPassed {
			rr := engineapi.RuleSkip(rule, engineapi.Mutation, "preconditions not met")
			responses = append(responses, *rr)
			continue
		}
