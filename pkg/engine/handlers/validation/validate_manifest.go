...
func (h validateManifestHandler) Process(
	...
	if err != nil {
		...
		return resource, handlers.WithResponses(engineapi.RuleError(rule, engineapi.Validation, "error occurred during manifest verification", err))
	}
	logger.V(3).Info("verifyManifest result", "verified", strconv.FormatBool(verified), "reason", reason)
	if !verified {
		return resource, handlers.WithResponses(engineapi.NewRuleResponse(rule, engineapi.Validation, reason, engineapi.RuleStatusFail))
	}
