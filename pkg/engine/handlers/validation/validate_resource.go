...
func (v *validator) validate(ctx context.Context) *engineapi.RuleResponse {
	if err := v.loadContext(ctx); err != nil {
		return engineapi.RuleError(v.rule, engineapi.Validation, "failed to load context", err)
	}
	preconditionsPassed, err := internal.CheckPreconditions(v.log, v.policyContext.JSONContext(), v.anyAllConditions)
	if err != nil {
		return engineapi.RuleError(v.rule, engineapi.Validation, "failed to evaluate preconditions", err)
	}
	if !preconditionsPassed {
		return engineapi.RuleSkip(v.rule, engineapi.Validation, "preconditions not met")
	}
	...
	if v.pattern != nil || v.anyPattern != nil {
		if err = v.substitutePatterns(); err != nil {
			return engineapi.RuleError(v.rule, engineapi.Validation, "variable substitution failed", err)
		}
...
func (v *validator) validateForEach(ctx context.Context) *engineapi.RuleResponse {
	...
	if applyCount == 0 {
		...
		return engineapi.RuleSkip(v.rule, engineapi.Validation, "rule skipped")
	}
	return engineapi.RulePass(v.rule, engineapi.Validation, "rule passed")
}
...
func (v *validator) validateElements(ctx context.Context, foreach kyvernov1.ForEachValidation, elements []interface{}, elementScope *bool) (*engineapi.RuleResponse, int) {
	...
	for index, element := range elements {
		...
		if err := engineutils.AddElementToContext(policyContext, element, index, v.nesting, elementScope); err != nil {
			...
			return engineapi.RuleError(v.rule, engineapi.Validation, "failed to process foreach", err), applyCount
		}
		...
		if err != nil {
			...
			return engineapi.RuleError(v.rule, engineapi.Validation, "failed to create foreach validator", err), applyCount
		}
		...
		if r == nil {
			...
			if r.Status == engineapi.RuleStatusError {
				...
				return engineapi.NewRuleResponse(v.rule, engineapi.Validation, msg, r.Status), applyCount
			}
			msg := fmt.Sprintf("validation failure: %v", r.Message)
			return engineapi.NewRuleResponse(v.rule, engineapi.Validation, msg, r.Status), applyCount
		}
	...
	return engineapi.RulePass(v.rule, engineapi.Validation, ""), applyCount
}
...
func (v *validator) validateDeny() *engineapi.RuleResponse {
	if deny, err := internal.CheckDenyPreconditions(v.log, v.policyContext.JSONContext(), v.deny.GetAnyAllConditions()); err != nil {
		return engineapi.RuleError(v.rule, engineapi.Validation, "failed to check deny preconditions", err)
	} else {
		if deny {
			return engineapi.NewRuleResponse(v.rule, engineapi.Validation, v.getDenyMessage(deny), engineapi.RuleStatusFail)
		}
		return engineapi.RulePass(v.rule, engineapi.Validation, v.getDenyMessage(deny))
	}
}
...
func (v *validator) validatePatterns(resource unstructured.Unstructured) *engineapi.RuleResponse {
	if v.pattern != nil {
		if err := validate.MatchPattern(v.log, resource.Object, v.pattern); err != nil {
			...
			if ok {
				...
				if pe.Skip {
					return engineapi.RuleSkip(v.rule, engineapi.Validation, pe.Error())
				}

				if pe.Path == "" {
					return engineapi.NewRuleResponse(v.rule, engineapi.Validation, v.buildErrorMessage(err, ""), engineapi.RuleStatusError)
				}

				return engineapi.NewRuleResponse(v.rule, engineapi.Validation, v.buildErrorMessage(err, pe.Path), engineapi.RuleStatusFail)
			}

			return engineapi.NewRuleResponse(v.rule, engineapi.Validation, v.buildErrorMessage(err, pe.Path), engineapi.RuleStatusError)
		}

		v.log.V(4).Info("successfully processed rule")
		msg := fmt.Sprintf("validation rule '%s' passed.", v.rule.Name)
		return engineapi.RulePass(v.rule, engineapi.Validation, msg)
	}
	...
	if v.anyPattern != nil {
		...
		if err != nil {
			return engineapi.RuleError(v.rule, engineapi.Validation, "failed to deserialize anyPattern, expected type array", err)
		}
		...
		if len(skippedAnyPatternErrors) > 0 && len(failedAnyPatternsErrors) == 0 {
			...
			return engineapi.RuleSkip(v.rule, engineapi.Validation, strings.Join(errorStr, " "))
			...
			return engineapi.RuleFail(v.rule, engineapi.Validation, msg)
		}
	}

	return engineapi.RulePass(v.rule, engineapi.Validation, v.rule.Validation.Message)
}
