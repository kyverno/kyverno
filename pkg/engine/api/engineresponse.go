...
// GetValidationFailureActionForRule resolves the effective failure action for the named rule:
// the rule's own action if it sets one, otherwise the spec-level override or spec-level action.
// Rules are resolved against the autogenerated rule set so names match the rules actually
// evaluated (autogen renames rules for pod controllers); an unresolvable name falls back to
// GetValidationFailureAction. Unlike that policy-wide accessor, the result does not depend on
// rule ordering when a policy mixes Enforce and Audit rules. An empty string is returned for a
// ValidatingAdmissionPolicy.
func (er EngineResponse) GetValidationFailureActionForRule(ruleName string) kyvernov1.ValidationFailureAction {
	pol := er.Policy()
	if pol.AsKyvernoPolicy() == nil {
		return ""
	}
	...
	return er.failureActionForRule(kpol.GetSpec(), rules, ruleName)
}
