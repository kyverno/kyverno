...
func buildFuzzContext(ff *fuzz.ConsumeFuzzer) (*PolicyContext, error) {
	...
	if len(autogen.Default(validateContext).ComputeRules(cpol, "")) == 0 {
...
func FuzzEngineValidateTest(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if len(autogen.Default(validateContext).ComputeRules(policy, "")) == 0 {
			return
		}
...
func FuzzMutateTest(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if len(autogen.Default(ctx).ComputeRules(policy, "")) == 0 {
			return
		}
