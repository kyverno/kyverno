...
func Test_VariableSubstitutionPatchStrategicMerge(t *testing.T) {
	...
	assert.Equal(t, len(er.PolicyResponse.Rules[0].ZPatches()), 1)
	t.Log(string(er.PolicyResponse.Rules[0].ZPatches()[0]))
	if !reflect.DeepEqual(expectedPatch, er.PolicyResponse.Rules[0].ZPatches()[0]) {
...
func Test_variableSubstitutionPathNotExist(t *testing.T) {
  ...
  "spec": {
	...
	er := testMutate(context.TODO(), nil, registryclient.NewOrDie(), policyContext, nil)
	assert.Equal(t, len(er.PolicyResponse.Rules), 1)
	assert.Assert(t, strings.Contains(er.PolicyResponse.Rules[0].ZMessage(), "Unknown key \"name1\" in path"))
}
...
func Test_chained_rules(t *testing.T) {
  ...
  "spec": {
	...
	assert.Equal(t, len(er.PolicyResponse.Rules[0].ZPatches()), 1)
	assert.Equal(t, len(er.PolicyResponse.Rules[1].ZPatches()), 1)

	assert.Equal(t, string(er.PolicyResponse.Rules[0].ZPatches()[0]), `{"op":"replace","path":"/spec/containers/0/image","value":"myregistry.corp.com/foo/bash:5.0"}`)
	assert.Equal(t, string(er.PolicyResponse.Rules[1].ZPatches()[0]), `{"op":"replace","path":"/spec/containers/0/image","value":"otherregistry.corp.com/foo/bash:5.0"}`)
}
...
func Test_mutate_nested_foreach(t *testing.T) {
	...
	assert.Equal(t, len(er.PolicyResponse.Rules), 1)
	assert.Equal(t, er.PolicyResponse.Rules[0].ZStatus(), engineapi.RuleStatusPass)
	assert.Equal(t, len(er.PolicyResponse.Rules[0].ZPatches()), 2)
	...
	for _, test := range tests {
		...
		for _, target := range test.targets {
			...
			for _, rr := range er.PolicyResponse.Rules {
				for i, p := range rr.ZPatches() {
					assert.Equal(t, test.patches[i], string(p), "test %s failed:\nGot %s\nExpected: %s", test.name, rr.ZPatches()[i], test.patches[i])
...
func Test_RuleSelectorMutate(t *testing.T) {
	...
	if !reflect.DeepEqual(expectedPatch1, er.PolicyResponse.Rules[0].ZPatches()[0]) {
		t.Error("rule 1 patches dont match")
	}
	if !reflect.DeepEqual(expectedPatch2, er.PolicyResponse.Rules[1].ZPatches()[0]) {
