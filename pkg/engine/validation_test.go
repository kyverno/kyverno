...
func TestValidate_image_tag_fail(t *testing.T) {
	...
	for index, r := range er.PolicyResponse.Rules {
		assert.Equal(t, r.ZMessage(), msgs[index])
	}
...
func Test_VariableSubstitution_NotOperatorWithStringVariable(t *testing.T) {
	...
	assert.Equal(t, er.PolicyResponse.Rules[0].ZMessage(), "validation error: rule not-operator-with-variable-should-alway-fail-validation failed at path /spec/content/")
}
...
func Test_VariableSubstitutionPathNotExistInAnyPattern_AllPathPresent_NonePatternSatisfy(t *testing.T) {
	...
	assert.Equal(t, er.PolicyResponse.Rules[0].ZStatus(), engineapi.RuleStatusFail)
	assert.Equal(t, er.PolicyResponse.Rules[0].ZMessage(),
...
func Test_VariableSubstitutionValidate_VariablesInMessageAreResolved(t *testing.T) {
	...
	er := testValidate(context.TODO(), registryclient.NewOrDie(), policyContext, cfg, nil)
	assert.Equal(t, er.PolicyResponse.Rules[0].ZStatus(), engineapi.RuleStatusFail)
	assert.Equal(t, er.PolicyResponse.Rules[0].ZMessage(), "The animal cow is not in the allowed list of animals.")
}
...
func Test_Flux_Kustomization_PathNotPresent(t *testing.T) {
	...
	for _, test := range tests {
		...
		for i, rule := range er.PolicyResponse.Rules {
			...
			assert.Equal(t, er.PolicyResponse.Rules[i].ZMessage(), test.expectedMessages[i], "\ntest %s failed\nexpected: %s\nactual: %s", test.name, test.expectedMessages[i], rule.ZMessage())
		}
	}
}
...
func testForEach(t *testing.T, policyraw []byte, resourceRaw []byte, msg string, status engineapi.RuleStatus, contextLoader engineapi.ContextLoaderFactory) {
	...
	if msg != "" {
		assert.Equal(t, er.PolicyResponse.Rules[0].ZMessage(), msg)
	}
}
