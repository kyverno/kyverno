...
func Test_denyFeatureIssue744(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			userInfo:      []byte(`{"roles":["kube-system:kubeadm:kubelet-config-1.17","kube-system:kubeadm:nodes-kubeadm-config"],"clusterRoles":["system:basic-user","system:certificates.k8s.io:certificatesigningrequests:selfnodeclient","system:public-info-viewer","system:discovery"],"userInfo":{"username":"kubernetes-admin","groups":["system:nodes","system:authenticated"]}}`),
			expectedError: true,
		},
		{
	...
	for _, testcase := range testcases {
		...
		err = json.Unmarshal(testcase.request, &request)
		if err != nil {
			t.Fatal(err)
		}

		var userInfo kyverno.RequestInfo
		err = json.Unmarshal(testcase.userInfo, &userInfo)
		if err != nil {
			t.Fatal(err)
		}
		...
		err = ctx.AddSA(userInfo.AdmissionUserInfo.Username)
		if err != nil {
			t.Fatal(err)
		}
		...
		resp := Validate(pc)
		if resp.IsSuccesful() != !testcase.expectedError {
			t.Errorf("Testcase has failed")
		}
	}
}
