...
func processImageValidationRule(ctx context.Context, log logr.Logger, rclient registryclient.Client, enginectx *api.PolicyContext, rule *kyvernov1.Rule, cfg config.Configuration) *api.RuleResponse {
	...
	if err != nil {
		return ruleResponse(*rule, api.Validation, err.Error(), api.RuleStatusError)
	}
	if len(matchingImages) == 0 {
		return ruleResponse(*rule, api.Validation, "image verified", api.RuleStatusSkip)
	}
	if err := LoadContext(ctx, log, rclient, rule.Context, enginectx, rule.Name); err != nil {
		...
		return ruleError(rule, api.Validation, "failed to load context", err)
	}

	preconditionsPassed, err := checkPreconditions(log, enginectx, rule.RawAnyAllConditions)
	if err != nil {
		return ruleError(rule, api.Validation, "failed to evaluate preconditions", err)
	}
	...
	if !preconditionsPassed {
		...
		return ruleResponse(*rule, api.Validation, "preconditions not met", api.RuleStatusSkip)
	}
		...
		for _, infoMap := range enginectx.JSONContext().ImageInfo() {
			for name, imageInfo := range infoMap {
				...
				if err := validateImage(enginectx, imageVerify, name, imageInfo, log); err != nil {
					return ruleResponse(*rule, api.ImageVerify, err.Error(), api.RuleStatusFail)
				}
			}
		}
	}
	...
	return ruleResponse(*rule, api.Validation, "image verified", api.RuleStatusPass)
}

func validateImage(ctx *api.PolicyContext, imageVerify *kyvernov1.ImageVerification, name string, imageInfo apiutils.ImageInfo, log logr.Logger) error {
	image := imageInfo.String()
...
func isImageVerified(resource unstructured.Unstructured, image string, log logr.Logger) (bool, error) {
	...
	ivm, err := api.ParseImageMetadata(data)
	if err != nil {
