...
func (nin NotInHandler) Evaluate(key, value interface{}) bool {
	...
	default:
		nin.log.V(4).Info("Unsupported type", "value", typedKey, "type", fmt.Sprintf("%T", typedKey))
		return false
	}
}
...
func (nin NotInHandler) validateValueWithStringPattern(key string, value interface{}) bool {
	...
	if invalidType {
		nin.log.V(4).Info("expected type []string", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
