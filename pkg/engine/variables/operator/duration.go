...
func durationCompareByCondition(key time.Duration, value time.Duration, op kyvernov1.ConditionOperator, log logr.Logger) bool {
	...
	case kyvernov1.ConditionOperators["DurationLessThan"]:
		return key < value
	default:
		log.V(4).Info(fmt.Sprintf("Expected operator, one of [DurationGreaterThanOrEquals, DurationGreaterThan, DurationLessThanOrEquals, DurationLessThan], found %s", op))
		return false
	}
}
...
func (doh DurationOperatorHandler) Evaluate(key, value interface{}) bool {
	...
	default:
		doh.log.V(4).Info("Unsupported type", "value", typedKey, "type", fmt.Sprintf("%T", typedKey))
		return false
	}
}
...
func (doh DurationOperatorHandler) validateValueWithIntPattern(key int64, value interface{}) bool {
	...
	case string:
		...
		return false
	default:
		doh.log.V(4).Info("Unexpected type", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
}
