...
func compareByCondition(key float64, value float64, op kyvernov1.ConditionOperator, log logr.Logger) bool {
	...
	case kyvernov1.ConditionOperators["LessThan"]:
		return key < value
	default:
		log.V(4).Info(fmt.Sprintf("Expected operator, one of [GreaterThanOrEquals, GreaterThan, LessThanOrEquals, LessThan, Equals, NotEquals], found %s", op))
		return false
	}
}
...
func (noh NumericOperatorHandler) Evaluate(key, value interface{}) bool {
	...
	default:
		noh.log.V(4).Info("Unsupported type", "value", typedKey, "type", fmt.Sprintf("%T", typedKey))
		return false
	}
}
...
func (noh NumericOperatorHandler) validateValueWithIntPattern(key int64, value interface{}) bool {
	...
	case string:
		...
		return false
	default:
		noh.log.V(4).Info("Expected type int", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
}
...
func (noh NumericOperatorHandler) validateValueWithFloatPattern(key float64, value interface{}) bool {
	...
	case string:
		...
		return false
	default:
		noh.log.V(4).Info("Expected type float", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
}
...
func (noh NumericOperatorHandler) validateValueWithVersionPattern(key semver.Version, value interface{}) bool {
	...
	default:
		noh.log.V(4).Info("Expected type string", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
}
