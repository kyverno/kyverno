...
func (anynin AnyNotInHandler) Evaluate(key, value interface{}) bool {
	...
	default:
		anynin.log.V(4).Info("Unsupported type", "value", typedKey, "type", fmt.Sprintf("%T", typedKey))
		return false
	}
}
...
func (anynin AnyNotInHandler) validateValueWithStringPattern(key string, value interface{}) bool {
	...
	if invalidType {
		anynin.log.V(4).Info("expected type []string", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
