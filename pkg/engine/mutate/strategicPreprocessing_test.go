...
func Test_preProcessStrategicMergePatch_BlankAnnotation(t *testing.T) {
	rawPolicy := []byte(`{"metadata":{"annotations":{"+(cluster-autoscaler.kubernetes.io/safe-to-evict)":true}, "labels":{"+(test-label)":"tl"}},"spec":{"volumes":[{"(hostPath)":{"path":"*"}}]}}`)
	...
	expected := `{"metadata":{"annotations":{"cluster-autoscaler.kubernetes.io/safe-to-evict": true},"labels":{"test-label":"tl"}},"spec":{"volumes":[{"name":"cache-volume"}]}}`
