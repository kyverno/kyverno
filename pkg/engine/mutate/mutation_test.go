...
func applyPatches(rule *types.Rule, resource unstructured.Unstructured) (*engineapi.RuleResponse, unstructured.Unstructured) {
	patcher, err := Mutate(logr.Discard(), rule, context.NewContext(jmespath.New(config.NewDefaultConfiguration(false))))
	if err != nil {
		return engineapi.RuleError(rule.Name, engineapi.Mutation, "failed to create patcher", err), resource
	}
	resource, response := ApplyPatchers(logr.Discard(), resource, *rule, patcher)
