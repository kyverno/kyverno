...
func (v *notaryVerifier) VerifySignature(ctx context.Context, opts images.Options) (*images.Response, error) {
	...
	targetDesc, outcomes, err := notation.Verify(notationlog.WithLogger(ctx, logging.NewDebugLogger(v.log.WithName("Notary Verifier Debug").V(4))), notationVerifier, parsedRef.Repo, remoteVerifyOptions)
	if err != nil {
