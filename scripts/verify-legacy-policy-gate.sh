...
if [ "${VERIFY_ALLOW_DESTRUCTIVE:-}" = "1" ]; then
  ...
  # This script deletes and recreates the five legacy CRDs to test the
  # absent-CRD case, which cascade-deletes any instances. A disposable CI
  # cluster has none of these CRDs yet (the gate runs before Kyverno is
  # installed), but a persistent or shared kind cluster could have them - with
  # or without instances - and deleting a CRD the script did not create removes
  # that API type (and any of its resources) from the operator's cluster. So
  # refuse if any of the five CRDs already exists, and fail closed if existence
  # can't be determined. Check the CRD itself rather than listing instances, so
  # a zero-instance CRD is still caught and there is no short-circuiting pipe.
  PREEXISTING=""
  for name in "${CRD_NAMES[@]}"; do
    if crd_out="$(kubectl get crd "${name}" -o name 2>&1)"; then
      PREEXISTING="${PREEXISTING} ${name}"
    elif ! printf '%s' "${crd_out}" | grep -qiE 'not ?found'; then
      fail "could not determine whether CRD ${name} exists on context '${CURRENT_CONTEXT}' (kubectl: ${crd_out}); refusing to run a script that deletes CRDs. Set VERIFY_ALLOW_DESTRUCTIVE=1 to override."
    fi
  done
  if [ -n "${PREEXISTING}" ]; then
    fail "refusing to run against context '${CURRENT_CONTEXT}': the legacy CRDs (${PREEXISTING# }) already exist and this script did not create them; deleting them to test the absent-CRD case would remove them (and any of their resources). Run against a clean, disposable cluster, or set VERIFY_ALLOW_DESTRUCTIVE=1 to override."
  fi
fi

# Pin every kubectl call from here on to the context the guard validated, so a
# context switch mid-run (another process running `kubectl config use-context`)
# can't redirect these destructive commands to a different, unvalidated cluster.
