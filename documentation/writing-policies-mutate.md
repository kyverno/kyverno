...
A Patch rule is similar to JSONPatch - RFC 6902.

Patch supports the following operations (in the 'op' field):
* **add**
* **replace**
* **remove**
...
This policy will replace or add the existing image and imagePullPolicy in the pod and will remove group label from metadata.
...
When the above policy is applied on the following resource:
