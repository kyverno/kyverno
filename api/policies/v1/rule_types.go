...
func (r *Rule) GetMutateExistingOnPolicyUpdate() bool {
	return r.Mutation.MutateExistingOnPolicyUpdate
}

// ValidateRuleType checks only one type of rule is defined per rule
func (r *Rule) ValidateRuleType(path *field.Path, namespaced bool) (errs field.ErrorList) {
	...
	if namespaced && r.HasValidate() {
		if len(r.Validation.ValidationFailureActionOverrides) > 0 {
			errs = append(errs, field.Forbidden(path.Child("validate").Child("validationFailureActionOverrides"), "Use of validationFailureActionOverrides is supported only with ClusterPolicy"))
		}
	}
	return errs
}
...
func (r *Rule) Validate(path *field.Path, namespaced bool, policyNamespace string, clusterResources sets.Set[string]) (errs field.ErrorList) {
	errs = append(errs, r.ValidateRuleType(path, namespaced)...)
